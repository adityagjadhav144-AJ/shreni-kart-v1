...
export function VerificationPanel({
  ...
  const userId = user?.uid || session?.user?.id || profile?.id || "";
  const verify = startVerification;
  const skip = skipVerification;
  ...
  async function run() {
    if (!consent) {
      toast.error("Please give your consent to continue.");
      return;
    }
    if (!testId.trim()) {
      toast.error("Please choose a demo verification ID.");
      return;
    }
    setBusy(true);
    try {
      const res = await verify({ data: { testId, consent: true, userId } });
      setResult(res);
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-3 rounded-2xl bg-gold/20 px-4 py-3">
        ...
        <p className="text-xs leading-relaxed text-[#5b3a12]">
          <b>Demo / Test verification.</b> This prototype is not connected to UIDAI. Never enter a
          real Aadhaar number — we do not accept or store Aadhaar numbers, images, OTPs or biometric
          data. Only the verification status and time are saved.
        </p>
      </div>
      ...
      <div className="rounded-3xl bg-card p-5 shadow-soft">
        ...
        <div className="mt-3 grid gap-2">
          ...
          >
            DEMO-FAIL-000{" "}
            <span className="text-[11px] font-normal opacity-75">· shows failure</span>
          </button>
        </div>
      </div>
      ...
      <label className="flex items-start gap-3 rounded-2xl bg-card px-4 py-3.5 shadow-soft">
        ...
        <span className="text-xs leading-relaxed text-muted-foreground">
          I agree to a demo identity check. I understand no Aadhaar data is collected and that only
          my verification status is stored with my artisan profile.
        </span>
      </label>
      ...
      {result?.status === "failed" ? (
        <div className="flex items-start gap-3 rounded-2xl bg-destructive/10 px-4 py-3 text-xs text-destructive">
          ...
          <span>
            {result.reason}. You can try again or continue and verify later — your account still
            works.
          </span>
        </div>
      ) : null}
